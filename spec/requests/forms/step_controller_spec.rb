...
RSpec.describe Forms::StepController, :capture_logging, type: :request do
  ...
  context "when setting logging context" do
    ...
    before do
      ...
      get form_page_path(form_id: 200, form_slug: form_data.form_slug, step_slug: page_id, mode:)
    end
  ...
  shared_examples "ordered steps" do
    it "redirects to first page when second request before first complete" do
      get form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2)
      expect(response).to redirect_to(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id))
    end

    it "redirects to first page when the page does not exist for the form" do
      get form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 99_999_999)
      expect(response).to have_http_status(:found)
      expect(response.location).to eq(form_page_url(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id))
    end
  end

  shared_examples "page with footer" do
    it "Displays the privacy policy link on the page" do
      get form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id)
  ...
  shared_examples "question page" do
    ...
    context "with a page that has a previous page" do
      it "Displays a link to the previous page" do
        ...
        get form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2)
        expect(response.body).to include(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id))
      end
    end
  ...
  shared_examples "a translatable page" do
    let(:english_url) { form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, locale: "en") }
    let(:welsh_url) { form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, locale: "cy") }
  ...
  shared_examples "a change answer page" do
    it "Displays a back link to the check your answers page" do
      get form_change_answer_path(form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, mode:)
    ...
    it "Passes the changing answers parameter in its submit request" do
      get form_change_answer_path(form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, mode:)
      expect(response.body).to include(save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, changing_existing_answer: true, answer_index: 1))
    end

    it_behaves_like "a translatable page" do
      let(:english_url) { form_change_answer_path(form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, mode:, locale: "en") }
      let(:welsh_url) { form_change_answer_path(form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, mode:, locale: "cy") }
    end
  end
  ...
  describe "#show" do
    ...
    context "when page has routing conditions" do
      ...
      context "when the routing has a cannot_have_goto_page_before_routing_page error" do
        ...
        context "when the route is a secondary skip" do
          ...
          it "returns a 422 response" do
            get form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 4)
            expect(response).to have_http_status(:unprocessable_content)
          end
    ...
    context "when page is repeatable" do
      ...
      it "shows the page as normal when there are no stored answers" do
        get form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: first_step_in_form.id, answer_index: 1)
        expect(response).to have_http_status(:ok)
      end

      it "returns 404 when given an invalid answer_index" do
        get form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: first_step_in_form.id, answer_index: 12)
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context "when the page is a file upload question" do
      ...
      context "when the question has already been answered" do
        ...
        it "redirects to the review file route" do
          expect(response).to redirect_to review_file_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id)
        end
      end
  ...
  describe "#save" do
    ...
    shared_examples "for validating answer" do
      context "when the form is invalid" do
        before do
          allow_any_instance_of(Flow::Context).to receive(:can_visit?).and_return(true)
          allow_any_instance_of(Flow::Context).to receive(:previous_step).and_return(OpenStruct.new(page_id: 1))
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: second_step_in_form.id), params: { question: { text: "" }, changing_existing_answer: false }
        end
        ...
        it "assigns a back link to the previous page" do
          expect(assigns(:back_link)).to eq(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id))
        end
      end
    end
    ...
    shared_examples "for redirecting after saving answer" do
      it "Redirects to the next page" do
        post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id), params: { question: { text: "answer text" }, changing_existing_answer: false }
        expect(response).to redirect_to(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2))
      end
      ...
      context "when changing an existing answer" do
        it "Redirects to the check your answers page" do
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, params: { question: { text: "answer text" }, changing_existing_answer: true })
      ...
      context "with the final page" do
        it "Redirects to the copy of answers page" do
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2), params: { question: { text: "answer text" } }
    ...
    context "with preview mode on" do
      ...
      context "with the first page" do
        it "does not log the first_page_save event" do
          expect(EventLogger).not_to receive(:log)
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id), params: { question: { text: "answer text" } }
        end
      ...
      context "with an subsequent optional page" do
        ...
        context "when an optional question is completed" do
          it "Logs the optional_save event with skipped_question as true" do
            ...
            post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2), params: { question: { text: "" } }
          end
        end
    ...
    context "when page has routing conditions" do
      ...
      it "redirects to the goto page if answer value matches condition" do
        post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id), params: { question: { selection: "Option 1" }, changing_existing_answer: false }
        expect(response).to redirect_to(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 3))
      end

      context "when the answer_value does not match the condition" do
        it "redirects to the next page in the journey" do
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id), params: { question: { selection: "Option 2" }, changing_existing_answer: false }
          expect(response).to redirect_to(form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: 2))
        end
      end
    ...
    context "when page is repeatable" do
      ...
      it "redirects to the add another answer page when given valid answer" do
        post save_form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: first_step_in_form.id, params: { question: { number: 12 } })
      ...
      it "shows 404 if an invalid answer_index is given" do
        post save_form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: first_step_in_form.id, answer_index: 3, params: { question: { number: 12 } })
        expect(response).to have_http_status(:not_found)
      end
      ...
      context "and is optional" do
        ...
        it "redirects to the next page when not given an answer" do
          post save_form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: first_step_in_form.id, params: { question: { number: nil } })
          expect(response).to redirect_to(form_page_path(mode:, form_id: form_data.form_id, form_slug: form_data.form_slug, step_slug: second_step_in_form.id))
        end
      end
    end
    ...
    context "when the page is a file upload question" do
      ...
      context "when a file was uploaded" do
        ...
        it "redirects to the review file route" do
          post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id), params: { question: }
          expect(response).to redirect_to review_file_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id)
        end
        ...
        context "when changing an existing answer" do
          it "includes the changing_existing_answer query parameter in the redirect" do
            post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, changing_existing_answer: true), params: { question: }
            expect(response).to redirect_to review_file_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, changing_existing_answer: true)
          end
        end
      end
    ...
    context "when the page is a selection question with a none of the above question" do
      ...
      before do
        ...
        post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, params:)
      end
      ...
      context "when there are more than 30 options" do
        ...
        context "when none of the above is selected" do
          ...
          it "redirects to the none of the above page" do
            expect(response).to redirect_to selection_none_of_the_above_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id)
          end
    ...
    context "when the page is a an exit question" do
      ...
      it "redirects to the exit page when exit page answer given" do
        post save_form_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id, params: { question: { selection: "Option 1" }, changing_existing_answer: false })
        expect(response).to redirect_to exit_page_path(mode:, form_id: 2, form_slug: form_data.form_slug, step_slug: first_page_id)
      end
