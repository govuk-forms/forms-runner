...
RSpec.describe Question::NumberComponent::View, type: :component do
  let(:form_document_step) { build :question_step, answer_type: "number" }
  let(:number) { nil }
  let(:question) do
  ...
  describe "when component is number field" do
    ...
    context "when the question has hint text" do
      let(:form_document_step) { build :question_step, :with_hints, answer_type: "number" }
    ...
    context "with unsafe question text" do
      let(:form_document_step) { build :question_step, answer_type: "number", question_text: "What is your name? <script>alert(\"Hi\")</script>" }
      let(:mode) { Mode.new("preview-draft") }
    ...
    context "when question has guidance" do
      let(:form_document_step) { build :question_step, :with_guidance, answer_type: "number" }
