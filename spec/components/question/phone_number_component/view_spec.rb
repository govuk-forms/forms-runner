...
RSpec.describe Question::PhoneNumberComponent::View, type: :component do
  let(:form_document_step) { build :question_step, answer_type: "phone_number" }
  ...
  describe "when component is phone number field" do
    ...
    context "when the question has hint text" do
      let(:form_document_step) { build :question_step, :with_hints, answer_type: "phone_number" }
    ...
    context "with unsafe question text" do
      let(:form_document_step) { build :question_step, answer_type: "phone_number", question_text: "What is your phone number? <script>alert(\"Hi\")</script>" }
      let(:mode) { Mode.new("preview-draft") }
    ...
    context "when question has guidance" do
      let(:form_document_step) { build :question_step, :with_guidance, answer_type: "phone_number" }
