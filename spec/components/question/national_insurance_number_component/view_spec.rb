...
RSpec.describe Question::NationalInsuranceNumberComponent::View, type: :component do
  let(:form_document_step) { build :question_step, answer_type: "national_insurance_number" }
  ...
  describe "when component is national insurance number field" do
    ...
    context "when the question has hint text" do
      let(:form_document_step) { build :question_step, :with_hints, answer_type: "national_insurance_number" }
    ...
    context "with unsafe question text" do
      let(:form_document_step) { build :question_step, answer_type: "national_insurance_number", question_text: "What is your name? <script>alert(\"Hi\")</script>" }
      let(:mode) { Mode.new("preview-draft") }
    ...
    context "when question has guidance" do
      let(:form_document_step) { build :question_step, :with_guidance, answer_type: "national_insurance_number" }
