...
RSpec.describe Condition do
  ...
  describe "#==" do
    ...
    context "when other condition was created from a different form document condition" do
      it "returns false" do
        other_condition = described_class.new(form_document_condition: build(:condition_resource))
  ...
  describe "#goto_page_id" do
    context "when goto_page_id is nil" do
      let(:form_document_condition) { build(:condition_resource, goto_page_id: nil) }
  ...
  describe "#exit_page_id" do
    context "when form document does not have exit_page_id" do
      let(:form_document_condition) do
        condition = build(:condition_resource, exit_page_id: nil)
    ...
    context "when exit_page_id is nil" do
      let(:form_document_condition) { build(:condition_resource, exit_page_id: nil) }
  ...
  describe "#new_style_exit_page?" do
    context "when condition has exit_page_id attribute and it is set" do
      let(:form_document_condition) do
        build(:condition_resource, exit_page_id: 10)
      end
    ...
    context "when condition has exit_page_id attribute and it is nil" do
      let(:form_document_condition) do
        build(:condition_resource, exit_page_id: nil)
      end
  ...
  describe "#exit_page?" do
    context "when condition has goto page id" do
      ...
      context "and does not have exit_page_id attribute" do
        let(:form_document_condition) do
          condition = build(:condition_resource, goto_page_id: Faker::Alphanumeric.alphanumeric(number: 8))
          condition.attributes.delete(:exit_page_id)
          condition
        end
    ...
    context "when condition has exit_page_id" do
      ...
      context "and does not have exit page content attributes" do
        let(:form_document_condition) do
          condition = build(:condition_resource, exit_page_id: 10)
          condition.attributes.delete(:exit_page_heading)
          condition.attributes.delete(:exit_page_markdown)
          condition
        end
    ...
    context "when condition has exit page content" do
      ...
      context "and does not have exit_page_id attribute" do
        let(:form_document_condition) do
          condition = build(:condition_resource, exit_page_heading: Faker::Lorem.sentence, exit_page_markdown: Faker::Lorem.paragraph)
          condition.attributes.delete(:exit_page_id)
          condition
        end
