...
RSpec.describe RepeatableStep, type: :model do
  ...
  let(:form) { build :form, form_slug: "form-slug", steps: [form_document_step, build(:question_step, id: 2)] }
  let(:form_document_step) { build :question_step }
  ...
  describe "#show_answer_in_json" do
    context "when the step has multiple answers" do
      let(:form_document_step) { build(:question_step, :with_name_settings, id: 2) }
      let(:questions) { [first_question, second_question] }
