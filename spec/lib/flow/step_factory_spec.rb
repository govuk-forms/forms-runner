...
RSpec.describe Flow::StepFactory do
  ...
  describe "#create_step" do
    ...
    context "when creating a regular step" do
      let(:form_document_step) { build_stubbed :question_step, id: "page-1", next_step_id: "page-2" }
      ...
      context "when it is the last step" do
        let(:form_document_step) { build :question_step, id: "page-1" }
    ...
    context "when creating a repeating step" do
      let(:form_document_step) { build :question_step, :with_repeatable, step_slug: "page-1" }
      let(:question) { instance_double(Question) }
    ...
    context "when creating the start step" do
      let(:start_page) { build :question_step, id: "page-1", next_step_id: "page-2" }
